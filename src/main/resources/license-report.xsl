<?xml version="1.0" encoding="UTF-8"?>
<xsl:stylesheet version="1.1" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                xmlns:fo="http://www.w3.org/1999/XSL/Format" exclude-result-prefixes="fo">
    <xsl:template match="LicenseReport">
        <fo:root xmlns:fo="http://www.w3.org/1999/XSL/Format">
            <fo:layout-master-set>
                <fo:simple-page-master master-name="simpleA4" page-height="29.7cm" page-width="21cm" margin-top="2cm"
                                       margin-bottom="2cm" margin-left="1cm" margin-right="1cm">
                    <fo:region-body/>
                </fo:simple-page-master>
            </fo:layout-master-set>
            <fo:page-sequence master-reference="simpleA4">
                <fo:flow flow-name="xsl-region-body">
                    <fo:block>
                        <xsl:if test="base64EncodedImage/text()">
                        <fo:external-graphic>
                            <xsl:attribute name="src">
                                <xsl:text>url('data:image/png</xsl:text>
                                <xsl:text>;base64,</xsl:text>
                                <xsl:value-of select="base64EncodedImage"/>
                                <xsl:text>')</xsl:text>
                            </xsl:attribute>
                        </fo:external-graphic>
                        </xsl:if>
                    </fo:block>
                    <fo:block/>
                    <fo:block font-size="12pt" font-weight="bold" space-after="5mm">
                        <xsl:value-of select="description"/>
                    </fo:block>
                    <fo:block/>
                    <fo:block font-size="12pt" font-weight="bold" space-after="5mm">Project Name:
                        <xsl:value-of select="projectName"/>
                    </fo:block>
                    <fo:block/>
                    <fo:block font-size="10pt">
                        <fo:table table-layout="fixed" width="100%" border-collapse="separate">
                            <fo:table-column column-width="7cm"/>
                            <fo:table-column column-width="10cm"/>
                            <fo:table-column column-width="2cm"/>
                            <fo:table-header background="#FFFF00">
                                <fo:table-row border="1pt solid black">
                                    <fo:table-cell border="inherit" padding="2px" background-color="#D9D9D9">
                                        <fo:block>Dependency</fo:block>
                                    </fo:table-cell>
                                    <fo:table-cell border="inherit" padding="2px" background-color="#D9D9D9">
                                        <fo:block>Licenses</fo:block>
                                    </fo:table-cell>
                                    <fo:table-cell border="inherit" padding="2px" background-color="#D9D9D9">
                                        <fo:block>License source</fo:block>
                                    </fo:table-cell>
                                </fo:table-row>
                            </fo:table-header>
                            <fo:table-body>
                                <xsl:apply-templates select="dependencyInformation"/>
                            </fo:table-body>
                        </fo:table>
                    </fo:block>
                </fo:flow>

            </fo:page-sequence>
        </fo:root>
    </xsl:template>
    <xsl:template match="dependencyInformation">
        <fo:table-row border="1pt solid black">
            <fo:table-cell border="inherit" padding="2px">
                <fo:block wrap-option="wrap">
                    <xsl:value-of select="name"/>
                </fo:block>
            </fo:table-cell>
            <fo:table-cell border="inherit" padding="2px">
                <fo:block wrap-option="wrap">
                    <xsl:value-of select="license"/>
                </fo:block>
            </fo:table-cell>
            <fo:table-cell border="inherit" padding="2px">
                <fo:block>
                    <xsl:value-of select="source"/>
                </fo:block>
            </fo:table-cell>
        </fo:table-row>
    </xsl:template>
</xsl:stylesheet>
